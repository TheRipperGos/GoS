...
function Soraka:Killsteal()
	...
	local target = Game.Hero(i)
		...
		if TRS.Items.Red.RedKS:Value() and redemption and Game.IsChatOpen() == false and myHero.pos:DistanceTo(target.pos) < 5500 then
			...
			if (target.health/target.maxHealth <= TRS.Items.Red.enemyHealth:Value() / 100) then
				...
				--Draw.Circle(Pos,50, 4, TRS.Drawings.wColor:Value())
				if Pos then
				...
				--[[if OnScreen(target) and target.pos:To2D().onScreen then
					Control.CastSpell(keybindings[redemption],target.pos)
				else
					SetMovement(false)
...
function Soraka:Items()
	...
	local Rspeed = 20
	local Rdelay = 0.05
	if TRS.Items.Red.RedMe:Value() and (myHero.health/myHero.maxHealth <= TRS.Items.Red.myHealth:Value() / 100) then

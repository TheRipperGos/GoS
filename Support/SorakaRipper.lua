...
function Soraka:LoadMenu()
	...
	--------- Menu LaneClear -------------------------------------------------------------------------------------------------------
  	...
  	self.Menu.Ripper.LaneClear:MenuElement({id = "HQ", name = "Min minions hit Q", value = 4, min = 1, max = 7})
	...
	--------- Menu Flee ------------------------------------------------------------------------------------------------------------
  	...
  	self.Menu.Ripper.Flee:MenuElement({id ="W", name = "Use Q", value = true, leftIcon = Icons.Q})
	...
	local hero = Game.Hero(i)
	if hero.team == myHero.team and not hero.isMe then
	self.Menu.Ripper.Heal.HP:MenuElement({id = hero.networkID, name = hero.charName.." max HP (%)", value = 65, min = 0, max = 100})
	end
	end
	end, 2)
	--------- Menu ULT -------------------------------------------------------------------------------------------------------------
  	...
  	self.Menu.Ripper.ULT:MenuElement({id = "Health", name = "Max Soraka Health (%)", value = 25, min = 0, max = 100})
	DelayAction(function()
	for i = 1,Game.HeroCount() do
	local hero = Game.Hero(i)
	if hero.team == myHero.team and not hero.isMe then
	...
	local hero = Game.Hero(i)
	if hero.team == myHero.team and not hero.isMe then
	self.Menu.Ripper.ULT.HP:MenuElement({id = hero.networkID, name = hero.charName.." max HP (%)", value = 45, min = 0, max = 100})
	end
	end
	end, 2)
	--------- Menu KS --------------------------------------------------------------------------------------------------------------
...
function Soraka:AllyCountEnemys(range)
	local heroesCount = 0
   	for i = 1,Game.HeroCount() do
        local enemy = Game.Hero(i)
        if  enemy.team ~= myHero.team and enemy.valid and enemy.pos:DistanceTo(ally.pos) < 600 then
        return true
        end
    	end
    	return false
...
function Soraka:LastQ()
	...
	local minion = Game.Minion(i)
    	...
    	if Qdamage >= minion.health and Qdamage >= self:HpPred(minion, 0.5) and (myHero.mana/myHero.maxMana >= self.Menu.Ripper.LastHit.Mana() / 100) and self:Ready(_Q) then
...
function Soraka:AutoR()
	...
	if (ally.health/ally.maxHealth <= self.Menu.Ripper.ULT.HP[hero.networkID]:Value() / 100) and not ally.isMe and enemy.pos:DistanceTo(ally.pos) < 600 then
...
function Soraka:SelfAutoR()
	...
	if self.Menu.Ripper.ULT.R:Value() == false then return end
	if (myHero.health/myHero.maxHealth <= self.Menu.Ripper.ULT.Health:Value() / 100) and self:Ready(_R) and myHero.pos:DistanceTo(target.pos) < 550 then
	Control.CastSpell(HK_R)
	end
end
